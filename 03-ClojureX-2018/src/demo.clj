...
;;
;; "let" example:
;; get a mix of 30 articles from your favorite feeds.
;;

(time
  ;; shall we add a p?
  (let [guardian (doall (xml-seq (xml/parse "https://www.theguardian.com/world/rss")))
        nytimes (doall (xml-seq (xml/parse "http://rss.nytimes.com/services/xml/rss/nyt/World.xml")))
        wsj (doall (xml-seq (xml/parse "http://www.wsj.com/xml/rss/3_7085.xml")))
        reuters (doall (xml-seq (xml/parse "http://feeds.reuters.com/reuters/UKTopNews")))]
    (count (concat
             (take 10 guardian)
...
(let [txt (p/slurp (io/file "../resources/war-and-peace.txt"))
      words (s/split txt #"\s+")]
  (take 5 (sort-by last > (time (frequencies (map s/lower-case words)))))
  (take 5 (sort-by last > (time (p/frequencies (map s/lower-case words)))))
  (take 5 (sort-by last > (time (p/frequencies words (map s/lower-case))))))
